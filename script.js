...
function addTodo(e){
  ...
  if( newTodo === '' ) showAlert('danger','Eksik Bilgi ...');
  else {
    todos.push({text: newTodo , completed: false});
    localStorage.setItem('todos', JSON.stringify(todos));
    showTodos();
    showAlert('success','İş başarıyla eklendi...');
  }
  textInput.value = '';
...
function deletedTodo(e){
  if( e.target.className === 'fas fa-times-circle' ){
    ...
    showAlert('success','İş Başarıyla Silindi...');
...
function deleteAllTodos(){
  ...
  localStorage.setItem('todos', JSON.stringify(todos));
  showTodos();
  showAlert('success','Bütün İşler Başarıyla Silindi...');
}
